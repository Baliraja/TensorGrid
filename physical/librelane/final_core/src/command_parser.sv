...
// Combinatorial lookup blocks replacing the function to eliminate check pass warnings
logic [2:0] rx_exp_bytes, curr_exp_bytes;

always_comb begin
    case(i_uart_rx_data[7:4])
        OP_QUANTIZE:      rx_exp_bytes = RX_QUANTIZE_BYTES;
        OP_DOT_PROD:      rx_exp_bytes = RX_DOT_PROD_BYTES;
        OP_LOAD_ELEMENT:  rx_exp_bytes = RX_LOAD_ELEMENT_BYTES;
        OP_READ_ELEMENT:  rx_exp_bytes = RX_READ_ELEMENT_BYTES;
        OP_SHIFT_ROW_COL: rx_exp_bytes = RX_SHIFT_ROW_COL_BYTES;
        OP_CLAMP:         rx_exp_bytes = RX_CLAMP_BYTES;
        default:          rx_exp_bytes = 3'd1;
    endcase

    case(current_op)
        OP_QUANTIZE:      curr_exp_bytes = RX_QUANTIZE_BYTES;
        OP_DOT_PROD:      curr_exp_bytes = RX_DOT_PROD_BYTES;
        OP_LOAD_ELEMENT:  curr_exp_bytes = RX_LOAD_ELEMENT_BYTES;
        OP_READ_ELEMENT:  curr_exp_bytes = RX_READ_ELEMENT_BYTES;
        OP_SHIFT_ROW_COL: curr_exp_bytes = RX_SHIFT_ROW_COL_BYTES;
        OP_CLAMP:         curr_exp_bytes = RX_CLAMP_BYTES;
        default:          curr_exp_bytes = 3'd1;
    endcase
end
...
always_ff @(posedge i_clk or negedge i_rst_n) begin
    if (!i_rst_n) begin
        ...
        if (byte_count == 4'd0) begin
            ...
            if (byte_count == (curr_exp_bytes - 3'd1)) begin
                byte_count <= 4'd0; 
            end else begin
