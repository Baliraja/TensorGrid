...
module integration_final_TB;
    ...
    begin
        if (actual === expected) begin
            pass_count++;
            $display("[%0t][CASE %0d] PASS: expected=%02h actual=%02h", $time, case_num, expected, actual);
        end else begin
            error_count++;
            $display("[%0t][CASE %0d] FAIL: expected=%02h actual=%02h", $time, case_num, expected, actual);
        end
    end
    endtask
    ...
    begin
        if (cond) begin
            pass_count++;
            $display("[%0t][CASE %0d] PASS: %s", $time, case_num, msg);
        end else begin
            error_count++;
            $display("[%0t][CASE %0d] FAIL: %s", $time, case_num, msg);
        end
    end
    endtask
    ...
    begin
        ...
        $display("[%0t][TB] Measured ce16 period = %0d clk cycles -> 1 bit = %0d clk cycles",
                 $time, clks_per_ce16, clks_per_bit);
    end
    endtask
    ...
    begin
        ...
        if (timeout) begin
            error_count++;
            $display("[%0t][CASE %0d] FAIL: UART TX timeout (no response)", $time, case_num);
        end else begin
    ...
    begin
        ...
        if (saw_start) begin
            receive_uart_byte(resp1, to1);
            $display("[%0t][INFO] zero-gap first response = %02h", $time, resp1);
            wait_tx_start(clks_per_bit * 15, saw_start);
            if (saw_start) begin
                receive_uart_byte(resp2, to2);
                $display("[%0t][INFO] zero-gap second response = %02h", $time, resp2);
            end else begin
                $display("[%0t][INFO] zero-gap second response byte dropped (uartTx busy)", $time);
            end
        end
        repeat (30) @(posedge clk);
    ...
    initial begin
        ...
        $dumpfile("integration_final_TB.vcd");
        $dumpvars(0, integration_final_TB);

        $display("[%0t] [TB] BAUD_FREQ=%0d BAUD_LIMIT=%0d (gcd-derived for %0d baud @ %0d Hz)",
                 $time, BAUD_FREQ, BAUD_LIMIT, UART_BAUD_RATE, CLK_FREQ);
